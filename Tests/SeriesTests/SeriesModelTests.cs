...
namespace Tests.SeriesTests
{
    ...
    public class SeriesModelTests
    {
        ...
        private AniListQuery AL_Query;
        private MangadexQuery MD_Query;
        
        [OneTimeSetUp]
        public void Setup()
        {
            AL_Query = new AniListQuery();
            MD_Query = new MangadexQuery();
        }
        ...
        {
            Assert.That(Series.CreateNewSeriesCard("Radiant", "MANGA", 18, 0, AL_Query, MD_Query, new ObservableCollection<string>()).ToJsonString(options), Is.EqualTo(File.ReadAllTextAsync(@"\Tsundoku\Tests\SeriesTests\SeriesTestData\Radiant.json").Result));
        }
        ...
        [Test]
        public void GetJapaneseAltTitle_Test()
        {
            Assert.That("俺だけレベルアップな件", Is.EqualTo(Series.GetAltTitle("ja", new MangadexQuery().GetSeriesTitleAsync("나 혼자만 레벨업").RootElement.GetProperty("data").EnumerateArray().ElementAt(0).GetProperty("attributes").GetProperty("altTitles").EnumerateArray().ToList())));
        }

        [Test]
        public void CreateCoverFilePath_Test()
        {
            JsonElement narutoQuery = JsonDocument.Parse(AL_Query.GetSeriesTitle("Naruto", "MANGA", 1)).RootElement.GetProperty("Media");

            Assert.That("Covers\\NARUTO_MANGA.jpg", Is.EqualTo(Series.CreateCoverFilePath(narutoQuery.GetProperty("coverImage").GetProperty("extraLarge").ToString(), narutoQuery.GetProperty("title").GetProperty("romaji").ToString(), "MANGA", narutoQuery.GetProperty("synonyms").EnumerateArray().ToList(), Constants.Site.AniList)));
        }
        ...
        public void IdenticalSeriesNames_GetSeriesByID_Test()
        {
            Assert.Multiple(() =>
            {
                Assert.That(JsonDocument.Parse(AL_Query.GetSeriesID(98282, "MANGA", 1)).RootElement.GetProperty("Media").GetProperty("title").GetProperty("romaji").ToString(), Is.EqualTo("Getsuyoubi no Tawawa"));
                Assert.That(JsonDocument.Parse(AL_Query.GetSeriesID(125854, "MANGA", 1)).RootElement.GetProperty("Media").GetProperty("title").GetProperty("romaji").ToString(), Is.EqualTo("Getsuyoubi no Tawawa"));
            });
        }
        ...
        {
            Assert.That(Series.GetSeriesStaff(JsonDocument.Parse(AL_Query.GetSeriesTitle("文豪ストレイドッグス", "MANGA", 1)).RootElement.GetProperty("Media").GetProperty("staff").GetProperty("edges"), "full", "Manga", "Bungou Stray Dogs", new System.Text.StringBuilder()), Is.EqualTo("Kafka Asagiri | Harukawa35"));
        }

        [Test] // Testing with Bakemonogatari
        public void GetSeriesStaff_ToManyIllustrators_Test()
        {
            Assert.That(Series.GetSeriesStaff(JsonDocument.Parse(AL_Query.GetSeriesTitle("化物語", "MANGA", 1)).RootElement.GetProperty("Media").GetProperty("staff").GetProperty("edges"), "full", "Manga", "Bakemonogatari", new System.Text.StringBuilder()), Is.EqualTo("Ito Oogure | NISIOISIN | VOFAN | Akio Watanabe"));
        }

        [Test]
        public void GetSeriesStaff_MultplieStaffForValidRole_Test()
        {
            Assert.That(Series.GetSeriesStaff(JsonDocument.Parse(AL_Query.GetSeriesTitle("나 혼자만 레벨업", "MANGA", 1)).RootElement.GetProperty("Media").GetProperty("staff").GetProperty("edges"), "full", "Manga", "Na Honjaman Level Up", new System.Text.StringBuilder()), Is.EqualTo("Seong-Rak Jang | Chu-Gong | So-Ryeong Gi | Hyeon-Gun"));
        }
        ...
        public void GetSeriesStaff_Anthology_Test()
        {
            ...
            Assert.That(Series.CreateNewSeriesCard("リコリス・リコイル 公式コミックアンソロジー リピート", "MANGA", 5, 0, AL_Query, MD_Query, new ObservableCollection<string>()).Staff["Romaji"], Is.EqualTo("Takeshi Kojima | Mekimeki | Nyoijizai | GUNP | Itsuki Takano | Ren Sakuragi | sometime | Ryou Niina | Ginmoku | Mikaduchi | Nikomi Wakadori | Miki Morinaga | Raika Suzumi | Ree | Itsuki Tsutsui | Utashima | Shirou Urayama | Bonryuu | Yasuka Manuma | Yuichi | Marco Nii | Nana Komado | Yuu Kimura | Sugar.Kirikanoko | Atto | Kasumi Fukagawa | Tiv | Sou Hamayumiba | Kanari Abe | Nachi Aono | Koruse"));
        }
