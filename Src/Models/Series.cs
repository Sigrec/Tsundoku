...
;
			...
			if ((!isMangaDexId || isAniListID) && seriesDataDoc != null)
			{
				...
				coverPath = !isRefresh ? AniList.CreateCoverFilePath(seriesData.GetProperty("coverImage").GetProperty("extraLarge").GetString(), romajiTitle, filteredBookType, ref dupeIndex) : string.Empty;
			...
			else if (isMangaDexId || bookType == Format.Manga) // MangaDex
			{
				...
				if (seriesDataDoc != null)
				{
					...
					coverPath = !isRefresh ? CreateCoverFilePath(coverLink, romajiTitle, GetCorrectFormat(countryOfOrigin), ref dupeIndex) : string.Empty;
